/*
Baserow API spec

For more information about our REST API, please visit [this page](https://baserow.io/docs/apis%2Frest-api).  For more information about our deprecation policy, please visit [this page](https://baserow.io/docs/apis%2Fdeprecations).

API version: 1.18.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package client

import (
	"encoding/json"
)

// checks if the FileImportJobJobReport type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FileImportJobJobReport{}

// FileImportJobJobReport Import error report.
type FileImportJobJobReport struct {
	// An object containing field in error by rows. The keys are the row indexes from original file and values are objects of errors by fields.
	FailingRows map[string]map[string][]string `json:"failing_rows"`
}

// NewFileImportJobJobReport instantiates a new FileImportJobJobReport object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFileImportJobJobReport(failingRows map[string]map[string][]string) *FileImportJobJobReport {
	this := FileImportJobJobReport{}
	this.FailingRows = failingRows
	return &this
}

// NewFileImportJobJobReportWithDefaults instantiates a new FileImportJobJobReport object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFileImportJobJobReportWithDefaults() *FileImportJobJobReport {
	this := FileImportJobJobReport{}
	return &this
}

// GetFailingRows returns the FailingRows field value
func (o *FileImportJobJobReport) GetFailingRows() map[string]map[string][]string {
	if o == nil {
		var ret map[string]map[string][]string
		return ret
	}

	return o.FailingRows
}

// GetFailingRowsOk returns a tuple with the FailingRows field value
// and a boolean to check if the value has been set.
func (o *FileImportJobJobReport) GetFailingRowsOk() (*map[string]map[string][]string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.FailingRows, true
}

// SetFailingRows sets field value
func (o *FileImportJobJobReport) SetFailingRows(v map[string]map[string][]string) {
	o.FailingRows = v
}

func (o FileImportJobJobReport) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FileImportJobJobReport) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["failing_rows"] = o.FailingRows
	return toSerialize, nil
}

type NullableFileImportJobJobReport struct {
	value *FileImportJobJobReport
	isSet bool
}

func (v NullableFileImportJobJobReport) Get() *FileImportJobJobReport {
	return v.value
}

func (v *NullableFileImportJobJobReport) Set(val *FileImportJobJobReport) {
	v.value = val
	v.isSet = true
}

func (v NullableFileImportJobJobReport) IsSet() bool {
	return v.isSet
}

func (v *NullableFileImportJobJobReport) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFileImportJobJobReport(val *FileImportJobJobReport) *NullableFileImportJobJobReport {
	return &NullableFileImportJobJobReport{value: val, isSet: true}
}

func (v NullableFileImportJobJobReport) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFileImportJobJobReport) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


